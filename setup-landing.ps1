# setup-landing.ps1
# Jalankan: powershell -ExecutionPolicy Bypass -File setup-landing.ps1

Write-Host "Creating landing page components..." -ForegroundColor Cyan

# Buat folder
New-Item -ItemType Directory -Force -Path "components\landing" | Out-Null

# ─── Navbar.tsx ───────────────────────────────────────────
Set-Content -Path "components\landing\Navbar.tsx" -Encoding UTF8 -Value @'
"use client";
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {["Fitur", "Harga", "Blog", "Bantuan"].map((item) => (
            <Link
              key={item}
        ...
        <div className="flex items-center gap-3">
          <Link href="/login" className="hidden md:block text-white/60 hover:text-white text-sm font-medium transition-colors duration-200">
            Masuk
          </Link>
          <Link href="/register" className="bg-gradient-to-r from-[#7C3AED] to-[#EC4899] text-white text-sm font-semibold px-5 py-2.5 rounded-full hover:opacity-90 transition-opacity duration-200 shadow-lg shadow-purple-500/25">
            Mulai Gratis
          </Link>
        </div>
      </div>
...
Set-Content -Path "components\landing\SocialProofSection.tsx" -Encoding UTF8 -Value @'
export default function SocialProofSection() {
  const stats = [
    { icon: "👥", value: "5.000+", label: "Creator Aktif", sub: "TikTok, IG, YouTube" },
    { icon: "💰", value: "Rp2 Miliar", label: "Produk Terjual", sub: "Sejak 2023" },
    { icon: "⚡", value: "50.000+", label: "Transaksi", sub: "Rata-rata tiap bulan" },
    { icon: "⭐", value: "4.9/5", label: "Rating Kepuasan", sub: "Dari 1.200+ ulasan" },
  ];
  return (
...
# ─── FeaturesSection.tsx ──────────────────────────────────
Set-Content -Path "components\landing\FeaturesSection.tsx" -Encoding UTF8 -Value @'
const features = [
  { icon: "🔗", title: "Link in Bio", desc: "Satu link untuk semua kontenmu. Tampil profesional di bio Instagram, TikTok, dan YouTube." },
  { icon: "🛍️", title: "Digital Product Store", desc: "Jual e-book, template, preset, kursus, dan produk digital lainnya langsung dari halamanmu." },
  { icon: "☕", title: "Creator Donations", desc: "Terima dukungan dari fans dengan sistem donasi yang simpel dan terintegrasi langsung." },
  { icon: "👑", title: "Membership", desc: "Buat komunitas eksklusif berbayar. Kelola subscriber dan konten premium dalam satu tempat." },
  { icon: "🤝", title: "Affiliate System", desc: "Buat program afiliasi sendiri. Ajak orang lain promosikan produkmu dan bagi komisi otomatis." },
  { icon: "📧", title: "Email List Builder", desc: "Kumpulkan email pengunjung dan bangun audiens jangka panjang yang kamu miliki sepenuhnya." },
];
...
Set-Content -Path "components\landing\HowItWorksSection.tsx" -Encoding UTF8 -Value @'
const steps = [
  { icon: "✏️", title: "Buat Halaman Bio", desc: "Daftar gratis dalam 30 detik. Pilih username, upload foto, dan halaman bio profesionalmu langsung jadi.", color: "from-violet-500 to-purple-600" },
  { icon: "📦", title: "Tambahkan Produk atau Donasi", desc: "Upload produk digitalmu, aktifkan tombol donasi, atau buat halaman membership eksklusif.", color: "from-purple-500 to-pink-600" },
  { icon: "💸", title: "Mulai Menghasilkan Uang", desc: "Share link bio ke semua platform. Fans klik, bayar, dan uang langsung masuk ke rekeningmu.", color: "from-pink-500 to-rose-500" },
];
...
# ─── PricingSection.tsx ───────────────────────────────────
Set-Content -Path "components\landing\PricingSection.tsx" -Encoding UTF8 -Value @'
import Link from "next/link";
...
const plans = [
  ...
  { name: "Creator", price: "39.000", fee: "3%", period: "/ bulan", badge: "Hemat 40%", popular: false, features: ["25 link aktif", "10 produk digital", "Analitik lengkap", "Custom subdomain", "Withdrawal 2x/bulan"], cta: "Mulai Creator", href: "/register" },
  { name: "Pro", price: "99.000", fee: "1.5%", period: "/ bulan", badge: "PALING POPULER", popular: true, features: ["Unlimited link", "Unlimited produk digital", "Real-time analytics", "Custom domain sendiri", "AI bio generator ✨", "Withdrawal kapan saja"], cta: "Mulai Pro", href: "/register" },
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-4">
          ...
          <p className="text-white/40 text-lg max-w-xl mx-auto mb-1">
            ...
            <span className="text-white/70 font-semibold line-through decoration-red-500">10–15%</span>
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-12 items-start">
          {plans.map((plan, i) => (
            <div key={i} className={`relative flex flex-col rounded-3xl p-6 transition-all duration-300 ${
              ...
              <div className={`inline-flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full mb-5 w-fit ${
                plan.popular ? "bg-purple-500/20 text-purple-300" : "bg-white/5 text-white/50"
              }`}>
                ...
                {plan.features.map((f, j) => (
                  <li key={j} className="flex items-start gap-2">
                    <span className={`mt-0.5 w-4 h-4 rounded-full flex-shrink-0 flex items-center justify-center text-xs ${
                      plan.popular ? "bg-purple-500/30 text-purple-300" : "bg-white/10 text-white/50"
                    }`}>✓</span>
...
Set-Content -Path "components\landing\TestimonialsSection.tsx" -Encoding UTF8 -Value @'
const testimonials = [
  { name: "Kirana Putri", platform: "Instagram · 120K Followers", avatar: "KP", grad: "from-violet-500 to-pink-500", quote: "Sejak pakai Weiiz, saya bisa jual template Canva langsung dari bio Instagram. Bulan pertama saja sudah balik modal langganan berkali-kali lipat!", earnings: "Rp3.2jt", earningLabel: "bulan pertama" },
  { name: "Rizky Aditya", platform: "TikTok · 380K Followers", avatar: "RA", grad: "from-cyan-500 to-blue-600", quote: "Weiiz yang paling smooth buat konten kreator Indonesia. Payment via QRIS dan transfer lokal, withdrawal cepat, dan dashboard-nya bersih banget.", earnings: "Rp7.8jt", earningLabel: "pendapatan bulanan" },
  { name: "Siti Nuraini", platform: "YouTube · 55K Subscriber", avatar: "SN", grad: "from-emerald-500 to-teal-600", quote: "Fitur membership-nya game-changer buat saya. Subscriber bisa akses konten eksklusif, dan saya punya pendapatan yang lebih stabil setiap bulannya.", earnings: "Rp5.5jt", earningLabel: "recurring bulanan" },
];
...
# ─── CTASection.tsx ───────────────────────────────────────
Set-Content -Path "components\landing\CTASection.tsx" -Encoding UTF8 -Value @'
import Link from "next/link";
        ...
        <div className="relative rounded-3xl overflow-hidden">
          ...
          <div className="relative z-10 px-8 md:px-16 py-16 md:py-20 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/register" className="bg-white text-purple-700 font-black px-8 py-4 rounded-2xl text-base hover:bg-white/95 transition-all duration-200 shadow-2xl hover:-translate-y-0.5">
                Daftar Gratis — Mulai Sekarang →
              </Link>
              <Link href="#harga" className="text-white/70 hover:text-white font-semibold px-6 py-4 rounded-2xl border border-white/20 hover:border-white/40 transition-all duration-200 text-sm">
                Lihat Paket Harga
              </Link>
            </div>
...
# ─── Footer.tsx ───────────────────────────────────────────
Set-Content -Path "components\landing\Footer.tsx" -Encoding UTF8 -Value @'
import Link from "next/link";
    ...
    <footer className="border-t border-white/5 py-12">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/20 text-xs">© 2024 Weiiz. Dibuat dengan ❤️ untuk creator Indonesia.</p>
...
# ─── HeroSection.tsx ──────────────────────────────────────
Set-Content -Path "components\landing\HeroSection.tsx" -Encoding UTF8 -Value @'
import Link from "next/link";
          ...
          <div className="relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-1 shadow-2xl">
            ...
            <div className="bg-gradient-to-b from-[#13131A] to-[#0d0d12] rounded-2xl p-8 flex flex-col items-center gap-4">
              ...
              <div className="w-full max-w-xs flex flex-col gap-2">
                {[
                  { label: "🛍️ Template Canva Viral", style: "from-violet-600/30 to-purple-600/30 border-violet-500/30" },
                  { label: "💌 Newsletter Mingguan", style: "from-pink-600/20 to-rose-600/20 border-pink-500/20" },
                  { label: "☕ Dukung Konten Saya", style: "from-amber-600/20 to-orange-600/20 border-amber-500/20" },
                  { label: "📦 E-Book Instagram Growth", style: "from-blue-600/20 to-cyan-600/20 border-blue-500/20" },
                ].map((item) => (
...
Write-Host "  git commit -m `"feat: add landing page components`"" -ForegroundColor White
